...
- Decide whether a real TEE integration is feasible and, if yes, replace the signed mock receipt adapter.
- Run one live API + worker pass against that Postgres instance and confirm migrations, ingestion, retrieval, and receipts all work outside the sqlite fallback.
...
- Add a CI path that starts Postgres, runs migrations, and executes the smoke test against the Postgres backend instead of sqlite fallback.
...
- Remove the sqlite fallback entirely once hosted Postgres validation is stable and no local-only demo path is needed.
