...
CohortVault is a privacy-preserving AI collaboration workspace for research and hackathon teams. It lets teams upload sensitive materials, run AI-assisted workflows inside a controlled execution boundary, and inspect signed receipts showing how outputs were produced.
...
- Supabase Storage
- OpenAI or Anthropic
- Signed runtime receipt adapter
...
This hackathon version uses a signed runtime receipt adapter by default. The architecture is TEE-ready, and the attestation layer can be upgraded to a real TEE provider post-hackathon.
...
An attested AI workspace for teams handling sensitive research, strategy, and credentials.
...
CohortVault is a privacy-preserving AI collaboration workspace. Teams upload sensitive materials into a scoped workspace, run tasks in `Secure Run` mode, and receive useful outputs plus signed runtime receipts. Secrets are delegated through a broker instead of being copied to users or models. Outputs can be redacted by role, and all activity is captured in an audit trail.
...
- Document upload and private retrieval
- Secure Run mode with signed execution receipts
- Secret broker for delegated access to protected resources
...
`Teams use AI on private materials every day, but current workflows are unsafe. CohortVault is an attested AI workspace for handling sensitive research, credentials, and startup materials.`
...
`Here the owner uploads a private pitch deck and research notes. The builder can ask a question in Secure Run mode, but only receives a redacted output. The system logs the run and generates a signed receipt showing the runtime, policy, and source materials used.`
